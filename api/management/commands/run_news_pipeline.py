...
class Command(BaseCommand):
    help = (
        "Runs the full news automation pipeline: Scrape -> Score -> Generate -> Publish"
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--limit", type=int, default=10, help="Max items to scrape per source"
        )
        parser.add_argument(
            "--publish-limit",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"[{timezone.now()}] Starting News Automation Pipeline")
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Ingested {added} items, skipped {skipped} duplicates"
                )
            )
            ...
            if run.status == "completed":
                ...
                self.stdout.write(
                    self.style.ERROR(f"Pipeline Run Failed: {run.error_message}")
                )

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Critical error in news pipeline: {str(e)}")
            )
            logger.error(f"News pipeline error: {e}", exc_info=True)

        self.stdout.write(
            self.style.SUCCESS(f"[{timezone.now()}] News Automation Pipeline Finished")
        )
