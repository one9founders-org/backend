...
class Tool(models.Model):
    ...
    # Pricing
    pricing_models = models.JSONField(default=list, blank=True, help_text="['free', 'paid']")
    pricing_tiers = models.JSONField(default=list, blank=True, help_text="[{'name': 'Pro', 'price': 20, 'billing': 'monthly'}]")
    ...
    ideal_for = models.JSONField(default=list, blank=True, help_text="e.g., ['early-stage', 'bootstrapped', 'SaaS']")
...
Only return JSON."""
    ...
    def __str__(self):
        return f"{self.user_name} - {self.tool.name}"
...
class UserFavorite(models.Model):
    ...
    tool = models.ForeignKey(Tool, on_delete=models.CASCADE, related_name='favorited_by')
    created_at = models.DateTimeField(auto_now_add=True)
...
class ToolSubmission(models.Model):
    ...
    enriched_data = models.JSONField(default=dict, blank=True, help_text="AI-generated data")
    
    # Admin
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending', db_index=True)
    admin_notes = models.TextField(blank=True)
    approved_tool = models.ForeignKey(Tool, on_delete=models.SET_NULL, null=True, blank=True, related_name='submission')
    ...
    def approve_and_create_tool(self):
        ...
        tool = Tool.objects.create(
            ...
            short_description=self.short_description or self.enriched_data.get('short_description', ''),
...
class Deal(models.Model):
    tool = models.ForeignKey(Tool, on_delete=models.CASCADE, related_name='deals', null=True, blank=True)
...
class News(models.Model):
    ...
    # Relations
    related_tools = models.ManyToManyField(Tool, blank=True, related_name='news_articles')
