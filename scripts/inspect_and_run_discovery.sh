...
if [ "${DISCOVERY_ACTION:-inspect-and-run}" = "inspect-and-run" ] || [ "${DISCOVERY_ACTION}" = "oss-catalog" ] || [ "${DISCOVERY_ACTION}" = "hn-catalog" ]; then
  echo "===== START RUN ====="
  docker compose exec -T web bash -lc '
    for pat in run_tool_discovery discover_india discover_oss_catalog discover_hn_catalog; do
  ...
  if [ "${DISCOVERY_ACTION}" = "oss-catalog" ]; then
    ...
    echo "Started detached discover_hn_catalog --max-new 100"
  else
